	...
	foreach ($list as $language) {
		if ($language->active) {
			if($language->sef == 'ua'){
				...
				$currentLink = substr($language->link, 3);
			}
		}
	}
	foreach ($list as $language) {
		if (!$language->active) {
			$language->link = '/' . $language->sef . $currentLink;
		}
	}
}
...
<?php if ($params->get('dropdown', 1) && !$params->get('dropdownimage', 0)) : ?>
	...
	<?php foreach ($list as $language) : ?>
		...
		<?php
		$dir = JLanguage::getInstance($language->lang_code)->isRtl() ? '"rtl"' : '"ltr"';
		$selected = $language->active ? 'selected="selected"' : '';
		?>
		<option dir=<?php echo $dir ?>
				value="<?php echo $language->link;?>"
			<?php echo $selected; ?>><?php echo $language->title_native;?></option>
	<?php endforeach; ?>
	</select>
	</form>
